...
func Store(db *sqlx.DB, def *model.Definition) error {
	...
	for _, f := range def.Funcs {
		_, err = tx.Exec(`
			INSERT INTO `+"`funcs`"+`(
				`+"`package_id`, `path`, `file`, `name`, `receiver`, `signature`, `complexity_cognitive`, `complexity_cyclomatic`"+`
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
			def.ID, f.File, f.File, f.Name, f.Receiver, f.Signature, f.Complexity.Cognitive, f.Complexity.Cyclomatic,
		)
		if err != nil {
			tx.Rollback()
			return err
